...
def test_indicate():
    ...
    retv = inst.control_in(
        request_type_bitmap_field=0xA1,
        request_id=64,
        request_value=0x0000,
        index=usb_iface,
        length=0x0001,
    )
    ...
    if isinstance(retv, bytes):
        ...
        assert (retv[1] == pyvisa.constants.StatusCode(0)) and (
            retv[0] == b"\x01"
        ), f"indicator pulse failed: retv={retv}"
...
rm = pyvisa.ResourceManager(VISA_LIB_PATH)
